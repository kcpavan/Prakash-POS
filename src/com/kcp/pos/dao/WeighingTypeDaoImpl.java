 ...
 * @author pbhimanna
 */
@Repository("weighingTypeDaoImpl")
@Service
public class WeighingTypeDaoImpl implements  WeighingTypeDao{
 private static final Log log = LogFactory.getLog(ItemsDaoImpl.class);
   ...
   public List<BagWeight> findAll() {
    ...
    public List<WeighingType> findListByCategory(Integer id) {
		log.debug("getting Items instance with id: " + id);
		try {
			Query instance = entityManager.createNamedQuery("WeighingTypeCategoryMapping.findByCategory")
                                .setParameter("id", id);
